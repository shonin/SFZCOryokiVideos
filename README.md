# SFZC Oryoki Videos
## Built in Vue

_built by Elliot Charney - @shonin - elliot.charney@sfzc.org_

Place the video files in `/public/config/videos`.
Place the additional resources in `/public/config/additional-resources`

View the files `/src/videoIndex.js` & `src/resourcesIndex.js` to see what the
names of the videos and resources need to be in those folders. 

### Project setup
clone the repo, navigate to the proper directory and run
```
npm install
```

You need to have node installed on your machine for this to work.

### Compiles and hot-reloads for development
```
npm run serve
```
this runs the app locally, it's meant for dev purposes

### Compiles and minifies for production
```
npm run build
```

### Customize configuration
See [Configuration Reference](https://cli.vuejs.org/config/).
